...
function normalizeResponseItem(raw, base) {
  ...
  switch (p.type) {
    case 'message': {
      ...
      if (!text) return null;
      if (p.role === 'user') return { ...base, kind: 'user_message', payload: { message: text } };
...
function dedupeEvents(events) {
  ...
  for (const ev of events) {
    if (ev.kind === 'user_message') {
      if (lastUser !== null && ev.payload.message === lastUser) continue;
      lastUser = ev.payload.message;
    } else if (ev.kind === 'agent_message') {
      if (lastAgent !== null && ev.payload.message === lastAgent) continue;
