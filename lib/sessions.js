    ...
    case 'task_complete':
      return {
        ...
        payload: {
          ...
          error: p.error || null,
          codexErrorInfo: p.codex_error_info || null,
        },
      };
    case 'token_count':
  ...
  switch (p.type) {
    case 'message': {
      ...
      if (p.role === 'user') {
        ...
        if (/^(?:# AGENTS\.md instructions|<(?:environment_context|skills_instructions|multi_agent_mode))\b/.test(text)) return null;
...
function listSessions() {
  ...
  for (const f of files) {
    ...
    for (const ev of events) {
      ...
      if (ev.kind === 'turn_context' && ev.payload.model) model = ev.payload.model;
      if (meta) break;
    }
    if (!meta) continue;
